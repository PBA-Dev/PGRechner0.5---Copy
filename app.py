...
from modules.child_module1 import child_module1
from modules.child_module2 import child_module2
from modules.child_module3 import child_module3
from modules.child_module4 import child_module4
from modules.child_module5 import child_module5
...
child_weighted_score_mapping_tables = {
    # 0 to 18 months
    "0-18": {
...
def map_child_raw_to_weighted_score(module_id, raw_score, age_group):
    """Maps raw score to weighted score for children based on predefined tables and age group."""
    try:
        module_id = int(module_id)
    except (ValueError, TypeError):
        current_app.logger.error(
            f"Invalid module_id type for child weighted score mapping: {module_id}"
        )
        return 0.0

    if age_group not in child_weighted_score_mapping_tables:
        current_app.logger.warning(
            f"Child weighted score mapping table not found for age_group: {age_group}"
        )
        return 0.0

    if module_id not in child_weighted_score_mapping_tables[age_group]:
        current_app.logger.warning(
            f"Child weighted score mapping table not found for module_id: {module_id} in age_group: {age_group}"
        )
        return 0.0

    mapping_table = child_weighted_score_mapping_tables[age_group][module_id]
    weighted_score = 0.0
    ...
    try:
        ...
        current_app.logger.warning(
            f"Invalid raw_score type for child weighted score mapping (Module {module_id}): {raw_score}"
        )
        return 0.0
...
def calculate_child_module5_raw_score(answers):
    """Calculate the raw score for module 5 for children."""
    answers = answers or {}
    total_score = 0
...
def start():
    ...
    try:
        ...
        dob_date = datetime.strptime(dob_iso, "%d.%m.%Y").date()
    except ValueError:
        try:
            # Try parsing DD/MM/YYYY
            dob_date = datetime.strptime(dob_iso, "%d/%m/%Y").date()
        except ValueError:
            # If both fail, dob_date remains None
            pass
...
def module_page(module_id):
    ...
    return render_template(
        ...
        child_weighted_score_mapping_tables=child_weighted_score_mapping_tables,
...
# --- Route for DISPLAYING child module page (GET requests) ---
@app.route(
    "/child_module/<int:module_id>", methods=["GET"], endpoint="child_module_page"
)
def child_module_page(module_id):
    if module_id not in all_child_modules or module_id < 1 or module_id > len(all_child_modules):
        flash("Ungültiges Modul angefordert.", "error")
        if "module_answers" in session and session["module_answers"]:
            first_answered = min(
                int(k) for k in session["module_answers"].keys() if k.isdigit()
            )
            return redirect(url_for("child_module_page", module_id=first_answered))
        return redirect(url_for("rechner"))
    ...
    for mid_str, answers in all_session_answers.items():
        mid = int(mid_str)
        if mid not in all_child_modules:
            continue

        if mid == 5:
            raw_score = calculate_module5_raw_score(answers) # Needs adjustment for child calculation
        else:
            raw_score = 0.0
            for q_key, answer_data in answers.items():
                if q_key not in ["notes", "visited"] and isinstance(answer_data, dict):
                    raw_score += answer_data.get("score", 0)
        temp_module_scores_raw[mid_str] = raw_score
        temp_module_scores_weighted[mid_str] = map_child_raw_to_weighted_score(mid, raw_score, age_group)
    ...
    return render_template(
        ...
        module=module_data,
        module_id=module_id,
        TOTAL_MODULES=len(all_child_modules),
        current_answers=current_answers,
        current_estimated_score=current_estimated_score,
        max_score=max_score,
        pflegegrad_thresholds=pflegegrad_thresholds,
        all_modules=all_child_modules,
        temp_module_scores_weighted=temp_module_scores_weighted,
        weighted_score_mapping_tables=weighted_score_mapping_tables,
        child_weighted_score_mapping_tables=child_weighted_score_mapping_tables,
...
# --- Route for HANDLING child module submission (POST requests) ---
@app.route(
    "/child_module/<int:module_id>", methods=["POST"], endpoint="child_module_page_submit"
)
def child_module_page_submit(module_id):
    if module_id not in all_child_modules:
        flash("Ungültiges Modul.", "error")
        return redirect(url_for("rechner"))
                    ...
                    if 0 <= option_index < len(options):
                        ...
                        if isinstance(selected_option, dict):
                            session["module_answers"][module_id_str][
                                ...
                                "question": question_text,
                                "answer_text": selected_option.get("text", "N/A"),
                                "selected_level": selected_option.get("text", "N/A"), # Store the selected level
                                "option_index": option_index,
                                "type": "standard",
                            }
                        else:
    ...
    next_module_id = module_id + 1
    if next_module_id > len(all_child_modules):
        return redirect(url_for("calculate"))
    else:
        return redirect(url_for("child_module_page", module_id=next_module_id))
...
def calculate():
    ...
    if age_in_months < 216:
        ...
        return calculate_adult_pg()

def calculate_adult_pg():
    ...
    # --- Check for special case in Module 1 ---
    special_case_active = False
    if '1' in all_answers and '1.B' in all_answers['1']:
        if all_answers['1']['1.B'].get('answer_text') == 'Ja':
...
def calculate_child_pg():
    ...
    for module_id_str, answers in all_answers.items():
        ...
        if module_id == 5:
            current_module_raw_score = calculate_child_module5_raw_score(answers)
            for q_key, answer_data in answers.items():
                if q_key not in ["notes", "visited"] and isinstance(answer_data, dict):
                    current_detailed_answers[q_key] = answer_data
        elif module_id == 3: # Module 3 is age-independent and uses direct scores
            for q_key, answer_data in answers.items():
                if q_key not in ["notes", "visited"] and isinstance(answer_data, dict):
                    current_module_raw_score += answer_data.get("score", 0)
                    current_detailed_answers[q_key] = answer_data
        else: # Modules 1, 2, 4, 6 - age-dependent scoring
            for i, question_data in enumerate(module_data.get("questions", [])):
                question_id = question_data.get("id")
                if not question_id:
                    continue

                # Get the assessed child's independence level for this question
                # Use question_index_str for modules 1,2,4,6 as they are indexed 0,1,2...
                # For module 4, question_id is like '4.4.1', so use that directly
                if module_id == 4:
                    assessed_level = answers.get(question_id, {}).get("selected_level")
                else:
                    assessed_level = answers.get(str(i), {}).get("selected_level")

                if assessed_level:
                    # Determine age-appropriate level
                    age_appropriate_level = "nicht erforderlich" # Default for questions not applicable at certain ages
                    if question_id in CHILD_DEVELOPMENT_DATA.get(module_id, {}):
                        for min_m, max_m, level in CHILD_DEVELOPMENT_DATA[module_id][question_id]:
                            if min_m <= age_in_months < max_m:
                                age_appropriate_level = level
                                break
                        # Special handling for 11+ years where it becomes 'selbständig'
                        if age_in_months >= 132 and age_appropriate_level == "nicht erforderlich": # 11 years = 132 months
                            age_appropriate_level = "selbständig" # Or "vorhanden/unbeeinträchtigt" for cognitive modules

                    # Calculate score based on deviation
                    score_key = (assessed_level, age_appropriate_level)
                    score = CHILD_SCORING_MATRIX.get(score_key, 0)

                    # Apply special module 4 weightings
                    if module_id == 4 and question_id in CHILD_MODULE4_SPECIAL_SCORING:
                        special_score = CHILD_MODULE4_SPECIAL_SCORING[question_id].get(score_key)
                        if special_score is not None:
                            score = special_score

                    current_module_raw_score += score
                    
                    # Store detailed answer with calculated score
                    if module_id == 4:
                        current_detailed_answers[question_id] = {
                            "question": question_data.get("text", ""),
                            "answer_text": assessed_level,
                            "score": score,
                            "selected_level": assessed_level,
                        }
                    else:
                        current_detailed_answers[str(i)] = {
                            "question": question_data.get("text", ""),
                            "answer_text": assessed_level,
                            "score": score,
                            "selected_level": assessed_level,
                    ...
                    # If no answer, ensure it's recorded as 0 score
                    if module_id == 4:
                        current_detailed_answers[question_id] = {
                            "question": question_data.get("text", ""),
                            "answer_text": "Keine Antwort",
                            "score": 0,
                            "selected_level": "Keine Antwort",
                        }
                    else:
                        current_detailed_answers[str(i)] = {
                            "question": question_data.get("text", ""),
        ...
        # Special handling for KF 4.4.0 for children 0-18 months
        if age_in_months <= 18 and module_id == 4:
            kf_4_4_0_answer = all_answers.get("4", {}).get("4.4.0", {}).get("answer_text")
            if kf_4_4_0_answer == "Ja":
                current_module_raw_score += KF_4_4_0_POINTS
                current_detailed_answers["4.4.0"] = {
                    "question": "Bestehen gravierende Probleme bei der Nahrungsaufnahme, die einen außergewöhnlich pflegeintensiven Hilfebedarf im Bereich der Ernährung auslösen?",
                    "answer_text": "Ja",
                    "score": KF_4_4_0_POINTS,
                }
            else:
                current_detailed_answers["4.4.0"] = {
                    "question": "Bestehen gravierende Probleme bei der Nahrungsaufnahme, die einen außergewöhnlich pflegeintensiven Hilfebedarf im Bereich der Ernährung auslösen?",
    ...
    for module_id_str, raw_score in module_scores_raw.items():
        module_id = int(module_id_str)
        if module_id in all_child_modules:
            module_scores_weighted[module_id_str] = map_child_raw_to_weighted_score(module_id, raw_score, age_group)
        else:
            module_scores_weighted[module_id_str] = 0.0
    ...
    m3_score = module_scores_weighted.get("3", 0.0)
    m4_score = module_scores_weighted.get("4", 0.0)
    m5_score = module_scores_weighted.get("5", 0.0)
    m6_score = module_scores_weighted.get("6", 0.0)

    # Special rule for children 0-18 months: Module 1, 2, 4, 6 are not considered
    if age_in_months <= 18:
        final_total_score = m3_score + m5_score
        # If KF 4.1.B is "Ja", add 90 points (already handled in raw score for module 1)
        # If KF 4.4.0 is "Ja", add 20 points (already handled in raw score for module 4)
    else:
        final_total_score += m1_score
        if m2_score >= m3_score:
            final_total_score += m2_score
    ...
    # Special rule for children 0-18 months: one Pflegegrad higher
    if age_in_months <= 18 and pflegegrad < 5: # Cannot go higher than PG5
        pflegegrad += 1
...
def result_page():
    ...
    is_child_calculation = results.get("is_child_calculation", False)
    modules_to_use = all_child_modules if is_child_calculation else all_modules
    return render_template(
        "result.html",
        results=results,
        user_info=user_info,
        all_modules=modules_to_use,
        is_child_calculation=is_child_calculation,
    )
