...
def calculate_and_determine_pflegegrad(answers: Dict[str, int]) -> Dict[str, float | int]:
    ...
    for question_id, score in answers.items():
        ...
        if module_id not in raw_scores:
            raw_scores[module_id] = 0
        raw_scores[module_id] += score

    return calculate_pflegerad(raw_scores)

def calculate_child_pflegerad(answers: Dict[str, int], age_months: int) -> Dict[str, float | int]:
    """Calculate the total score and determine the Pflegegrad for a child."""
    raw_scores = {}
    for question_id, score in answers.items():
        module_id_str, question_number = question_id.split('.')
        module_id = int(module_id_str)
        
        if module_id not in raw_scores:
            raw_scores[module_id] = 0

        # Get the expected score for the child's age
        expected_score = 0
        if f"module_{module_id}" in child_development_data and question_id in child_development_data[f"module_{module_id}"]:
            for min_age, max_age, expected in child_development_data[f"module_{module_id}"][question_id]:
                if min_age <= age_months < max_age:
                    expected_score = expected
                    break
        
        # The score is the difference between the assessed score and the expected score
